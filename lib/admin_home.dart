...
// print(_workerEmailController.text);
// print(workerStaffId);
...
class _ManagerHomeScreenState extends State<ManagerHomeScreen>
  ...
  // Add Section / Add Task controllers
  ...
  List<Map<String, dynamic>> _sections = [];
  List<Map<String, dynamic>> _tasks = [];
  List<Map<String, dynamic>> _workerEmails = [];
  ...
  Future<void> _loadSections() async {
    ...
    try {
      ...
      _showError('Error loading sections: ${e.toString()}');
    }
  }
  ...
  Future<void> _loadtasks() async {
    ...
    try {
      ...
      _showError('Error loading tasks: ${e.toString()}');
    }
  }

  // Load worker emails from Staff table (role = 4)
  Future<void> _loadWorkerEmails() async {
    ...
    try {
      final response = await Supabase.instance.client
          ...
          .order('email');
      setState(() {
        _workerEmails = List<Map<String, dynamic>>.from(response);
        _isLoadingEmails = false;
      });
    } catch (e) {
      setState(() => _isLoadingEmails = false);
      _showError('Error loading worker emails: ${e.toString()}');
    }
  }

  // Load assigned tasks from Supabase
  Future<void> _loadAssignedTasks() async {
    setState(() => _isLoadingTasks = true);
    try {
      final managerStaffId = widget.staffData['staff_id'] as int;
      ...
      for (var task in response) {
        ...
        final workerTask = WorkerTask(
          id: task['task_id'].toString(),
          workerName: workerEmail.split('@')[0],
          workerEmail: workerEmail,
          taskTitle: taskTitle,
          section: sectionTitle,
          status: status == 'completed' ? 'Completed' : 'In Progress',
      ...
      setState(() => _isLoadingTasks = false);
      _showError('Error loading assigned tasks: ${e.toString()}');
    }
  }
  ...
  void _showAddSectionModal() {
    ...
    showModalBottomSheet(
      ...
      backgroundColor: Colors.transparent,
      builder: (context) => StatefulBuilder(
        builder: (context, setModalState) => _buildBottomSheetContainer(
          context: context,
          ...
          child: Column(
            ...
            children: [
              _buildTextField(
                ...
                label: 'Section Title',
                hint: 'e.g. Warehouse A – Row 3',
                icon: Icons.location_on,
              ),
              const SizedBox(height: 24),
              _buildSubmitButton(
                label: 'Save Section',
                onPressed: () => _saveSection(context),
              ),
              const SizedBox(height: 24),
            ],
          ),
  ...
  Future<void> _saveSection(BuildContext context) async {
    if (_newSectionTitleController.text.trim().isEmpty) {
      _showError('Please enter a section title');
      return;
    }
    ...
    try {
      ...
      await Supabase.instance.client.from('sections').insert({
        'section_title': _newSectionTitleController.text.trim(),
      });
      ...
      if (mounted) Navigator.pop(context);
      _showError('Error saving section: ${e.toString()}');
    }
  }
  ...
  Future<void> _saveTask(BuildContext context) async {
    ...
    try {
      ...
      // Fetch all existing task_ids to determine the next one
      final existing = await Supabase.instance.client
          .from('tasks')
          .select('task_id')
          .order('task_id', ascending: false);

      // Parse numeric part from IDs like T001, T002, ...
      int maxNum = 0;
      ...
      // Generate next ID, e.g. T006
      final nextId = 'T${(maxNum + 1).toString().padLeft(3, '0')}';

      await Supabase.instance.client.from('tasks').insert({
        'task_id': nextId,
        'title': _newTaskTitleController.text.trim(),
        if (_newTaskDescController.text.trim().isNotEmpty)
      ...
      if (mounted) Navigator.pop(context);
      _showError('Error saving task: ${e.toString()}');
    }
  }

  // ─────────────────────────────────────────────
  //  ASSIGN TASK MODAL (updated with email dropdown)
  // ─────────────────────────────────────────────
  void _showAssignTaskModal() {
    _selectedDate = null;
    _selectedTime = null;
          ...
          child: Column(
            ...
            children: [
              ...
              _buildDropdown<String>(
                value: _selectedtaskId,
                label: 'Task',
                hint: _isLoadingtasks ? 'Loading tasks...' : 'Select Task',
                icon: Icons.task_alt,
                isHighlighted: _selectedtaskId != null,
                items: _tasks.map((task) {
                  return DropdownMenuItem<String>(
                    value: task['task_id'] as String,
                    child: Text(task['title'] as String),
                  );
                }).toList(),
                onChanged: _isLoadingtasks
                    ...
                    : (value) {
                        setModalState(() {
                          _selectedtaskId = value;
                          _selectedtaskTitle = _tasks.firstWhere(
                              (t) => t['task_id'] == value)['title'] as String;
                        });
                      },
              ),
              const SizedBox(height: 16),

              // Worker Email Dropdown
              _buildDropdown<String>(
                value: _selectedWorkerEmail,
                label: 'Worker Email',
                hint: _isLoadingEmails
                    ? 'Loading workers...'
                    : 'Select Worker Email',
                icon: Icons.email,
                isHighlighted: _selectedWorkerEmail != null,
                items: _workerEmails.map((w) {
                  return DropdownMenuItem<String>(
                    value: w['email'] as String,
                    child: Text(
                      w['email'] as String,
                ...
                onChanged: _isLoadingEmails
                    ? null
                    : (value) {
                        setModalState(() {
                          _selectedWorkerEmail = value;
                          _workerEmailController.text = value ?? '';
                        });
                      },
              ),
              const SizedBox(height: 16),
              ...
              _buildDropdown<int>(
                ...
                hint: _isLoadingSections
                    ...
                    : 'Select Section',
                icon: Icons.location_on,
                isHighlighted: _selectedSectionId != null,
                items: _sections.map((section) {
                  return DropdownMenuItem<int>(
                    value: section['section_id'] as int,
                    child: Text(section['section_title'] as String),
                ...
                onChanged: _isLoadingSections
                    ...
                    : (value) {
                        setModalState(() {
                          _selectedSectionId = value;
                          _selectedSectionTitle = _sections.firstWhere(
                                  (s) => s['section_id'] == value)[
  ...
  Future<void> _assignTask(BuildContext context) async {
    ...
    try {
      ...
      final workerResponse = await Supabase.instance.client
          .from('Staff')
          .select('staff_id')
          .eq('email', _selectedWorkerEmail!)
          .eq('role', 4)
          .maybeSingle();

      if (workerResponse == null) {
        if (mounted) Navigator.pop(context);
        _showError('Worker not found with this email');
        return;
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      drawer: AdminDrawer(staffData: widget.staffData),
      body: Column(
        children: [
          // Header
          Container(
      ...
      bottomNavigationBar: Container(
        ...
        child: BottomNavigationBar(
          ...
          items: const [
            ...
            BottomNavigationBarItem(
                icon: Icon(Icons.warehouse), label: 'Products'),
            BottomNavigationBarItem(
                icon: Icon(Icons.people), label: 'Workers'),
            BottomNavigationBarItem(
                icon: Icon(Icons.assistant), label: 'IRIS AI'),
          ],
          onTap: (index) {
            switch (index) {
              case 0:
              ...
              case 1:
                Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (_) => WarehouseProductsScreen(staffData: widget.staffData)),
                );
                break;
              case 2:
                Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (_) => WorkersManagementScreen(
                          staffData: widget.staffData)),
                );
                break;
              case 3:
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (_) => ChatScreen( staffData: widget.staffData,role:"manager")),
                );
                break;
            }
          },
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          _buildTaskDetail(
                              Icons.person_outline, task.workerName),
                          const SizedBox(height: 8),
                          _buildTaskDetail(
                              Icons.location_on_outlined, task.section),
                          const SizedBox(height: 8),
                          _buildTaskDetail(
                            Icons.calendar_today,
                            '${task.date.day}/${task.date.month}/${task.date.year}',
